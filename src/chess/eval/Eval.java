...
public class Eval {
    ...
    public float evaluate(Board board, boolean white) {
        float points = 0;
        if (white && board.whiteWon() == 0) points += 50;
        if (!white && board.whiteWon() == 1) points += 50;
        if (white && board.whiteWon() == 1) points -= 50;
        if (!white && board.whiteWon() == 0) points -= 50;
        if (board.whiteWon() == -2) points -= 20;
        for (int i = 0; i < 8; i ++) {
            for (int j = 0; j < 8; j ++) {
    ...
    public ArrayList<HashMap<String, Board>> findPositions(Board board, boolean white, Integer depth, Integer time, Long finTime, String firstMove) {
        ...
        for (String move : cleanBestMoves) {
            ...
            if (depth != null) {
                ...
                positions.addAll(findPositions(pos, !white, null, (int)(time - timeElapsed)/5, finTime, firstMove));
            }
        }
        return positions;
    }
    public String findMove(Board board, boolean white, Integer depth, Integer time) {
        ...
        if (depth != null) {
            ...
            Board newBoard;
            for (String move : nextMoves) {
                newBoard = board.newBoardWithmove(move);
                String kingPos = newBoard.kingSquare(white);
                boolean stop = false;
                for (String nextMove : newBoard.nextPositions(!white, false)) {
                    if (kingPos == null) {
                        logger.warning(kingPos);
                        logger.warning(board.toString());
                    }
                ...
                if (move == null) continue;
                Board pos = board.newBoardWithmove(move);
                positions.addAll(findPositions(pos, !white, null, (time-10)/nextMoves.length, System.currentTimeMillis()+time-100, move));
            }
        }
        ...
        for (HashMap<String, Board> map : positions) {
            for (Map.Entry<String, Board> entry : map.entrySet()) {
                ...
                groupedPositions.computeIfAbsent(key, _k -> new ArrayList<>()).add(newBoard);
            }
        }
        // Now find best
